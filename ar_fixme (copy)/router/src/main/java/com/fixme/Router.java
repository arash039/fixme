...
public class Router {
		...
		public void completed(Integer result, ClientAttachment attachment) {
			try {
				if (result != -1) {
					...
					System.out.println("something received from market!");
				} else {
					System.out.println("nothing received from market!");
	...
	class BrokerHandler implements CompletionHandler<Integer, ClientAttachment> {
		...
		public void completed(Integer result, ClientAttachment attachment) {
			try {
				System.out.println("in brokerhandler");
				if (result != -1) {
					attachment.buffer.flip();
					int limit = attachment.buffer.limit();
					byte[] bytes = new byte[limit];
					...
					System.out.println("something received from broker!");
				} else {
					System.out.println("nothing received from broker");
					attachment.client.close();
					attachment.client = null;
				}
			} catch (IOException e ) {
	...
	public class MarketCompletionHandler implements CompletionHandler<AsynchronousSocketChannel, AsynchronousServerSocketChannel> {
		...
		//private static int marketsIndex = 0;
		//private static final int MAX_CLIENTS = 999999;
	...
	public class BrokerCompletionHandler implements CompletionHandler<AsynchronousSocketChannel, Void> {
		...
		public BrokerCompletionHandler(Map<String, ClientAttachment> brokers, AsynchronousServerSocketChannel brokerChannel) {
			System.out.println("iside constructor");
		...
		public void completed(AsynchronousSocketChannel client, Void attachment) {
			System.out.println("iside completed");
			if (client.isOpen()) {
		...
		private void registerBroker(AsynchronousSocketChannel client) {
			try {
				if (brokerIndex < MAX_CLIENTS) {
					...
					System.out.println("before read from broker");
		...
		public void run() {
			try {
				...
				if (clientAttachment != null && clientAttachment.client != null) {
					...
					// ClientAttachment sendClient = markets.get(senderId);
					// if (sendClient != null)
					// 	sendClient.client.write(ByteBuffer.wrap("Broker has disconnected.\n".getBytes())).get();
